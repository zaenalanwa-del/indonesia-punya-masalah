...
const boot=()=>{
  ...
  document.addEventListener('click',async e=>{
    ...
    if(sub){e.preventDefault();e.stopImmediatePropagation();const label=sub.dataset.submenu||sub.textContent.trim();try{if(window.NK_INTERACTIONS?.handleSubmenu){window.NK_INTERACTIONS.handleSubmenu(label);return;}const box=document.querySelector('#modal'),body=document.querySelector('#mb');if(box&&body){body.innerHTML='<h2>'+label.replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]))+'</h2><div class="adCard"><b>Nuansa Kita</b><p>Fitur sedang memuat data publik. Silakan tunggu beberapa detik lalu buka kembali menu ini.</p></div>';box.classList.add('open');}}catch(err){console.warn('[submenu-rescue]',err);const box=document.querySelector('#modal'),body=document.querySelector('#mb');if(box&&body){body.innerHTML='<h2>'+label.replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]))+'</h2><p>Fitur publik belum siap menampilkan data. Data sedang disiapkan.</p>';box.classList.add('open');}}return;}
