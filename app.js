...
function isKnownAdmin(){return sessionIsAdmin===true}
...
function initMainMap(){const mainMap=document.querySelector('#problemMap');if(!mainMap)return;if(window.L){const rows=[...(portal?.tables?.problems||[]),...(portal?.tables?.citizen_reports||[]),...(portal?.live_incidents||[])].filter(x=>Number.isFinite(Number(x.latitude))&&Number.isFinite(Number(x.longitude)));initLiveProblemMap(rows,mainMap)}else{setTimeout(initMainMap,1800)}}
...
async function openPublicIssue(x){
 const el=document.getElementById('reportDetail');const list=document.getElementById('masalah');if(!el||!list)return;
 const im=publicIssueImage(x);const resolved=await resolvePublicIssueMedia(x);const urls=resolved.length?resolved:(im.urls.length&&!im.urls.some(storageMediaPath)?im.urls:[im.fallback]);const isReal=resolved.length>0||im.urls.some(u=>/^https?:\/\//i.test(u))&&!x.is_demo;
...
setTimeout(async()=>{try{await getSession();if(await forceAdminRedirect())return;renderAuth()}catch(e){console.warn('[auth-boot]',e)}},80);
defer(()=>trackSiteVisit(),2500);
defer(()=>loadPublicAds(),1800);
defer(()=>loadPortal(),900);
